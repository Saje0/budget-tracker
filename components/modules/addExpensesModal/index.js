...
function Index({ showModal, setShowModal, setGetData }) {
  ...
  const handleSubmit = (event) => {
    ...
    const dataLocalstorage = localStorage?.getItem?.("expensesData"); //  get data from local storage
    const dataArray = JSON?.parse?.(dataLocalstorage); //  parse data from local storage
    const data = Array.isArray(dataArray)
      ? [
          ...dataArray, // 👈️ spread local storage data and add the new item
          {
    ...
    localStorage.setItem("expensesData", JSON.stringify(data)); //  add data to local storage
  ...
  return (
    <Modal width={"width-[800px]"} show={showModal} onClose={setShowModal}>
      <span className="text-xl font-bold text-center">Add Expenses</span>
